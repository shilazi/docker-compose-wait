...
pub struct Parameter {
    /// Appending pass-through arguments to the WAIT_COMMAND
...
pub fn parse_command<S: Into<String>>(
    ...
    if let Some(args) = (!parameter.args.is_empty()).then_some(parameter.args) {
...
pub fn config_from_env(parameter: Parameter) -> Config {
    Config {
        hosts: env_reader::env_var("WAIT_HOSTS", "".to_string()),
        paths: env_reader::env_var("WAIT_PATHS", "".to_string()),
        command: parse_command(env_reader::env_var("WAIT_COMMAND", "".to_string()), parameter)
...
mod test {
    ...
    fn config_should_use_default_values() {
        ...
        set_env("", "", "10o", "10", "", "abc", "");
        let config = config_from_env(Parameter::default());
    ...
    fn should_panic_when_given_an_invalid_command() {
        let _guard = TEST_MUTEX.lock().unwrap();
        set_env("", "", "", "", "", "", "a 'b");
        config_from_env(Parameter::default());
    }
    ...
    fn parse_command_fails_when_command_is_invalid() {
        assert!(parse_command(" intentionally 'invalid", Parameter::default()).is_err())
    }

    #[test]
    fn parse_command_returns_none_when_command_is_empty() {
        for c in &["", " \t\n\r\n"] {
            let p = parse_command(c.to_string(), Parameter::default()).unwrap();
            assert!(p.is_none());
        }
    }
    ...
    fn parse_command_handles_commands_without_args() {
        let (command, command_string) = parse_command("ls".to_string(), Parameter::default()).unwrap().unwrap();
    ...
    fn parse_command_handles_commands_with_args() {
        let (command, command_string) = parse_command("ls -al".to_string(), Parameter::default()).unwrap().unwrap();
    ...
    fn parse_command_discards_leading_and_trailing_whitespace() {
        let (command, command_string) = parse_command("     hello world    ".to_string(), Parameter::default())
            .unwrap()
            .unwrap();
    ...
    fn parse_command_strips_shell_quotes() {
        let (command, command_string) =
            parse_command(" find . -type \"f\" -name '*.rs' ".to_string(), Parameter::default())
                .unwrap()
                .unwrap();
